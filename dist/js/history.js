...
function setActiveFile(filename) {
  ...
  if (targetedFile != void 0)
    targetedFile.isActive = true;
  saveFiles(files);
}
function deleteFile(filename) {
  ...
  if (!activeFileExist && newFiles.length !== 0)
...
// dist/js/deletingAnimation.js
var ANIMATION_PIECES_INCREMENT = 3;
...
var MAKING_FILE_CONTAINER_RED_DELAY = 1e3;
var numberOfCallsAnimatePiecesFunction = getNumberOfCallsOfAnimatePiecesFunction(0, 0, getPieceColumnCount() * 24, 0);
var averageAnimatePiecesTime = ANIMATION_DELAY.max / 5 * 1.3;
var deleteFileTimeConsumption = MAKING_FILE_CONTAINER_RED_DELAY + ANIMATION_DURATION + numberOfCallsAnimatePiecesFunction * averageAnimatePiecesTime;
function performDeletingVisually(fileElement) {
...
function decreaseAnimationDelay(animationDelay) {
  if (animationDelay <= ANIMATION_DELAY.min)
    return ANIMATION_DELAY.min;
...
// dist/js/history.js
var savedFilesContainer = document.querySelector("[data-saved-files-container]");
var fileContainerTemplate = document.getElementById("file-container-template");
var editors = [];
$("document").ready(() => {
...
function handleDeleteFile(fileContainerElement) {
  ...
  const deleteFileTimeConsumption2 = performDeletingVisually(fileContainerElement);
  checkWhetherFilesCountIs0(deleteFileTimeConsumption2);
}
...
function renderFile({name, editorMode, text}) {
  ...
  const newEditor = new Editor_default(editorElement, getCurrentColorTheme(), true);
